...
function Sheet() {
  ...
  const onCellClick = (row: number, col: number, e: React.MouseEvent) => {
    ...
    else { dispatch(setSel({ row, col })); dispatch(setRange(null)); }
  };
  ...
  const startResizeCol = (idx: number, e: React.MouseEvent) => {
    e.preventDefault();
    const startX = e.clientX, startW = colW[idx];
  ...
  const startResizeRow = (idx: number, e: React.MouseEvent) => {
    e.preventDefault();
    const startY = e.clientY, startH = rowH[idx];
    const onMove = (me: MouseEvent) => {
  ...
  if (sel) return cells[`${sel.row},${sel.col}`]?.formula || cells[`${sel.row},${sel.col}`]?.display || '';
  return '';
  };
    ...
    <div className="row" style={{ height: rowH[index] }}>
      ...
      {Array(cols).fill(0).map((_, col) => (
        <Cell
          ...
          onContextMenu={(r, c, e) => { e.preventDefault(); setMenu({ row: r, col: c, x: e.clientX, y: e.clientY }); }}
          editing={editing}
  ...
  return (
    <div className="app">
      <div className="toolbar">
        ...
        <div className="save-status">{saveStatus === 'saved' ? 'Сохранено' : saveStatus === 'saving' ? 'Сохранение...' : ''}</div>
      </div>
      <FormulaBar value={getFormulaValue()} onChange={handleFormulaChange} onEnter={handleFormulaEnter} />
      <div className="table-container">
        <div className="table">
          ...
          <Virtuoso style={{ height: 700 }} totalCount={rows} itemContent={i => <RowRenderer index={i} />} />
        </div>
      </div>
      {menu && (
        <Menu
          x={menu.x} y={menu.y}
          onAddRow={() => { dispatch(addRow(menu.row)); setMenu(null); }}
          onAddCol={() => { dispatch(addCol(menu.col)); setMenu(null); }}
          onDeleteRow={() => { dispatch(delRow(menu.row)); setMenu(null); }}
          onDeleteCol={() => { dispatch(delCol(menu.col)); setMenu(null); }}
